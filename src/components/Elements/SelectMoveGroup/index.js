...
import {Group, Text} from 'react-konva-traceroll';
...
export function linesInsidePolygon(arrlines, polygon) {
	...
	for(let i = 0, n = arrlines.length; i < n; i++) {
		...
		//for line brush
		if (attrs.originalPoints) {
			points = attrs.originalPoints;
		}
		for (let j = 0, k = points.length; j < k - 1; j += 2) {
			if (inside(points[j] + line.getX(), points[j+1] + line.getY(), polygon)) {
				checkArr[i] = true;
	...
	for(let i = 0, n = arrlines.length; i < n; i++) {
		if (!checkArr[i]) {
			...
			for (let j = 0, k = points.length; j < k - 1; j += 4) {
				if (lineCross(points[j] + line.getX(), points[j+1] + line.getY(), 
									points[j+2] + line.getX(), points[j+3] + line.getY(), polygon)) {
					selectedLine.push(line);
					break;
				}
			}
